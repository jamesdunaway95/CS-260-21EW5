...
private:
	// Node struct that takes a bid object, a node point for the next item in the list, and a head.
	struct Node {
		Bid bid;
		Node *next;
...
LinkedList::LinkedList() {
	// Head is set to null as a new list will always be empty.
	head = nullptr;
	tail = nullptr;
}
...
void LinkedList::Remove(string bidId) {
	...
	if (temp == nullptr){
		return;
    // If head matches and there are not other nodes.
	} else if (head->bid.bidId.compare(bidId) == 0 && head->next == nullptr) {
		head = nullptr;
		tail = nullptr;
		delete temp;
